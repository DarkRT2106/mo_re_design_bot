...
# ВАЖНО: замени на настоящий Telegram ID менеджера
...
@dp.callback_query_handler(lambda c: c.data == "printing_menu")
...
@dp.callback_query_handler(lambda c: c.data.startswith("printing:"))
...
@dp.callback_query_handler(lambda c: c.data == "back_printing")
...
@dp.callback_query_handler(lambda c: c.data.startswith("order:service:"))
...
@dp.callback_query_handler(lambda c: c.data.startswith("order:printing:"))
...
if message.text:
    ...
    client_message = message.caption
else:
    client_message = "Клиент отправил файл/медиа без текста."
